...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!user) return;
    // Log user data from localStorage to see what fields are available
    ...
    const fetchProfile = async () => {
      ...
      if (!token) return;
      try {
        const res = await fetch("https://back-end-sawu.onrender.com/admin/admin-sec", {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (res.ok) {
          const data = await res.json();
          const adminsList = data.admins || data.data || (Array.isArray(data) ? data : []);
          if (adminsList.length > 0) {
            const match = adminsList.find(a => a.email === user?.email);
            setProfile(match || adminsList[0]);
          }
        } else {
          // 403 = sec-admin can't access this endpoint, use user data from localStorage instead
          console.warn("admin-sec API returned", res.status, "— using localStorage user data as profile");
          setProfile(user);
        }
      } catch (err) {
            ...
            <div className="px-6 pb-6 relative">
              ...
              <div className="mt-12">
                ...
                <div className="space-y-4 pt-4 border-t border-gray-100">
                  ...
                  <div>
                    <p className="text-xs font-bold text-gray-400 tracking-wider uppercase mb-1">Assigned Commune</p>
                    <div className="flex items-center gap-2 text-sm font-semibold text-gray-800">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-emerald-500"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>
                      {profile?.quartier_nom ? profile.quartier_nom : "Commune Not Assigned"}
                    </div>
                  </div>
                  <div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="p-5 border-l-4 border-emerald-500 bg-emerald-50/50 rounded-r-xl">
                <p className="text-sm text-gray-700 leading-relaxed">
                  Welcome to your localized security oversight portal. Please ensure you regularly review and action any pending association approvals within your jurisdiction. Maintain close communication with local Maire accounts for optimal event coordination.
                </p>
                <p className="text-xs font-medium text-gray-400 mt-3">— Regional Security Command</p>
              </div>
            </div>
          </div>
