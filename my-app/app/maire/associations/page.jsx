  ...
  const fetchAssociations = async () => {
    ...
    try {
      const [pendingRes, approvedRes] = await Promise.all([
        fetch(`${BASE_URL}/admin/associations/pending`, { headers: { Authorization: `Bearer ${token}` } }).catch(() => null),
        fetch(`${BASE_URL}/admin/associations/approved`, { headers: { Authorization: `Bearer ${token}` } }).catch(() => null)
      ]);

      if (pendingRes && pendingRes.ok) {
        const pendingData = await pendingRes.json();
        setPending(pendingData.associations || (Array.isArray(pendingData) ? pendingData : []));
      } else {
        setPending([]);
      }
      ...
      if (approvedRes && approvedRes.ok) {
        ...
        setApproved(approvedData.associations || (Array.isArray(approvedData) ? approvedData : []));
      } else {
        setApproved([]);
      }
    } catch (err) { 
