...
import MaireLayout from "../../components/MayorLayout";
...
// Lightweight fetch helper that returns JSON and throws "HTTP <status>" on non-OK responses.
...
function getQuartierId(user) {
  return user?.quartier_id || user?.quartierId || user?.quartier?.id || user?.maire?.quartier_id || null;
}
...
export default function Dashboard() {
  ...
  const refreshDashboard = async () => {
    ...
    const token = getToken();
    const fetchers = [
      { key: "core",   path: "/statistics/CoreMetrics",                       setter: setCore },
      { key: "impact", path: "/statistics/ImpactMetrics",                     setter: setImpact },
      { key: "donations", path: "/statistics/DonationsByCategory",            setter: (d) => setDonations(Array.isArray(d) ? d : []) },
      { key: "topCities", path: "/statistics/top-cities",                     setter: (d) => setCities(Array.isArray(d) ? d : []) },
    ...
    const timeout = setTimeout(() => setReloading(false), 15000);
      ...
      fetchers.map(({ key, path, setter }) =>
        fetchJSON(scopedStatsPath(path, user), token)
          ...
          .catch((e) => {
            ...
            else if (e.message !== "HTTP 403") {
              setLoadError(true);
            }
            console.debug("Dashboard: failed", key, e.message);
          })
      )
    ).finally(() => {
        ...
        <div className="two-col">
          ...
          <div className="card">
            <div className="card-header">
              ...
              <button className="pill-btn" onClick={() => router.push("/maire/analytics")}>
