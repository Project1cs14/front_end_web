  ...
  const executeConfirmAction = async () => {
    ...
    try {
      ...
      if (confirmModal.type === "suspend") {
        ...
        requestInit.body = JSON.stringify({ reason: "Comportement abusif" });
      }
      ...
      if (confirmModal.type === "delete") {
        setUsers((prev) => prev.filter((u) => (u.user_id || u.id) !== confirmModal.userId));
        setDetailTarget(null);
        showToast("User account deleted successfully", "success");
      } else if (confirmModal.type === "suspend") {
        setUsers((prev) =>
          prev.map((u) => ((u.user_id || u.id) === confirmModal.userId ? { ...u, is_active: 0 } : u))
        );
        showToast("User suspended successfully", "success");
      } else if (confirmModal.type === "resume") {
        setUsers((prev) =>
          prev.map((u) => ((u.user_id || u.id) === confirmModal.userId ? { ...u, is_active: 1 } : u))
        );
                      ...
                      <td style={{ padding: "12px 16px", textAlign: "right" }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 6 }}>
                          ...
                          {isActive ? (
                            <button
                              ...
                              style={{
                                ...
                                alignItems: "center",
                                justifyContent: "center",
                                cursor: actionLoading === (user.user_id || user.id) ? "not-allowed" : "pointer",
                                transition: "all .2s",
                                opacity: actionLoading === (user.user_id || user.id) ? 0.5 : 1,
                              }}
                              onMouseEnter={(e) => { if (actionLoading !== (user.user_id || user.id)) e.target.style.background = "#fed7aa"; }}
                            ...
                            <button
                              onClick={() => handleResumeUser(user.user_id || user.id)}
                              disabled={actionLoading === (user.user_id || user.id)}
                              ...
                              onMouseEnter={(e) => { if (actionLoading !== (user.user_id || user.id)) e.target.style.background = "#dcfce7"; }}
                          ...
                          <button
                            ...
                            onMouseEnter={(e) => { if (actionLoading !== (user.user_id || user.id)) e.target.style.background = "#fee2e2"; }}
