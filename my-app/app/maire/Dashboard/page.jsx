...
export default function Dashboard() {
  ...
  // Read user from storage only on the client (inside useEffect) to avoid hydration mismatch
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      if (!token) return;
      try {
        const res = await fetch("https://back-end-sawu.onrender.com/admin/maire", {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (res.ok) {
          const data = await res.json();
          if (data.maires && data.maires.length > 0) {
            // Match the logged-in user by email to get the correct profile
            const match = data.maires.find(m => m.email === user?.email);
            setProfile(match || data.maires[0]);
          }
        }
      } catch (err) {
    ...
    const fetchStats = async () => {
      ...
      try {
        const [donationsRes, associationsRes, usersRes] = await Promise.all([
          fetch("https://back-end-sawu.onrender.com/admin/donation/maire", { headers: { Authorization: `Bearer ${token}` } }),
  ...
  const handleLogout = async () => {
    ...
    // Clear session immediately — logout is never blocked by a network failure
    ...
    // Notify the server in the background, silently ignore any network error
    if (token) {
      fetch('https://back-end-sawu.onrender.com/auth/logout', {
        method: 'POST',
        headers: {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Welcome back, <span className="font-semibold text-gray-700">{profile?.name || user?.name}</span>. Here is what's happening today.
            </p>
          </div>
          <button
            ...
            <div className="px-6 pb-6 relative">
              ...
              <div className="mt-12">
                ...
                <div className="space-y-4 pt-4 border-t border-gray-100">
                  ...
                  <div>
                    <p className="text-xs font-bold text-gray-400 tracking-wider uppercase mb-1">Commune / Wilaya</p>
                    <div className="flex items-center gap-2 text-sm font-semibold text-gray-800">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-[#1a1f5e]"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
                      {profile?.quartier_nom ? `${profile.quartier_nom}${profile.wilaya ? `, ${profile.wilaya}` : ''}` : "District Not Assigned"}
                    </div>
                  </div>
                  <div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="p-5 border-l-4 border-[#1a1f5e] bg-blue-50/30 rounded-r-xl">
                <p className="text-sm text-gray-700 leading-relaxed">
                  Welcome to your municipal monitoring dashboard. You have read-only access to view and track all logistical operations, donations, and association movements within your delegated commune.
                </p>
                <p className="text-xs font-medium text-gray-400 mt-3">— Mayoral Governance Support</p>
              </div>
            </div>
          </div>
