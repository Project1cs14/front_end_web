...
// ── Main Page ──────────────────────────────────────────────────────────────
export default function MaireDonationsPage() {
  ...
  const fetchDonations = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(BASE_URL + "/admin/donation/maire", {
  ...
  const filtered = donations.filter((d) => {
    ...
    const matchSearch = !q ||
      ...
      d.donateur_name?.toLowerCase().includes(q) ||
      d.donateur_email?.toLowerCase().includes(q) ||
      ...
      <div className="min-h-screen w-full bg-[#f4f6fb] p-6">
        ...
        <div className="grid grid-cols-5 gap-4 mb-6">
          {[
            ...
            { label: "Reserved", value: stats.reserved, color: "text-amber-500", bg: "bg-emerald-50" },
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                ...
                <th className="px-4 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Donor</th>
                ...
                filtered.map((donation, index) => (
                  <tr key={donation.id} className="hover:bg-blue-50/30 transition-colors group">
                    ...
                    <td className="px-4 py-3.5 text-gray-600">
                      {donation.created_at ? new Date(donation.created_at).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : "—"}
                    </td>
                    <td className="px-4 py-3.5">
                      <span className={"inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full " +
